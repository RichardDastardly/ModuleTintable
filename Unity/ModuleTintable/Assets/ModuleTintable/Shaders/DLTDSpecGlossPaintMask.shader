Shader "DLTD/SpecGlossPaintMask" {
	Properties{
		...
		[Space]
		[Header(Maps)]
		_MainTex("AO Greyscale R, Gloss B, spec A", 2D) = "white" {} 
		...
		_Shininess("Specular tightness", Range(0.03, 1)) = 0.078125
...
#include "Tint.cginc"
			...
			void surf(Input IN, inout SurfaceOutput o)
			{
				...
				// premultiplied output for Overlay blending - multiply if Overlay lumi is < 0.5, otherwise screen
				mult = 2 * overlay.r * paintFinal.rgb;
				screen = 1 - 2 * (1 - overlay.r)*(1 - paintFinal.rgb);
				o.Albedo = lerp(mult, screen, round(overlay.r)) * _BurnColor;
		
				o.Emission = emission;

				o.Gloss = overlay.b;
				o.Specular = overlay.a;
				o.Normal = normal;
